...
from checkLocations import checkLocations
import math
...
# delete old files
    ...
    final_keywords = checkKeywords()
    print(f"In request_jobs.py --> final_keywords: {final_keywords}")
    with open('keywords.txt', 'w') as file:
    # Write each item in the list on a new line
        for item in final_keywords:
            file.write(item + '\n')

    final_locations = checkLocations()
    print(f"In request_jobs.py --> final_locations: {final_locations}")
    ...
    format_2_docx(final_keywords, final_locations)

    time.sleep(2)
    print("Hi, I'm formating (with docx library) json to docx for 5 seconds...")
    # formating json file into word document
    format_json_2_docx(final_keywords, final_locations)
